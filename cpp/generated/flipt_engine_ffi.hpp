...
class NativeFliptEngineFfi : public jsi::HostObject {
private:
...
protected:
  ...
  jsi::Value cpp_uniffi_internal_fn_func_ffi__arraybuffer_to_string(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_ffi_flipt_engine_ffi_uniffi_contract_version(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);

public:
  NativeFliptEngineFfi(
      jsi::Runtime &rt,
      std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
  virtual ~NativeFliptEngineFfi();

  /**
   * The entry point into the crate.
   *
   * React Native must call `NativeFliptEngineFfi.registerModule(rt,
   ...
   * be the case.
   *
   * Clients should call `NativeFliptEngineFfi.unregisterModule(rt)` after final
   * use where possible.
   */
